...
CONFIGSAFE_DOCKERFILE_PATTERNS+=(
  ...
  'apt-get[[:space:]]+install[[:space:]]+((?!--no-install-recommends).)*$|low|DF-011|apt-get install without --no-install-recommends|Add --no-install-recommends to reduce image size and attack surface'
  ...
  'apt-get[[:space:]]+install.*&&[[:space:]]*(?!.*rm[[:space:]]+-rf).*$|low|DF-020|apt-get install without cleanup — increases image size|Add && rm -rf /var/lib/apt/lists/* after apt-get install'
)
...
CONFIGSAFE_CICD_PATTERNS+=(
  ...
  'upload-artifact.*(?!retention-days)|low|CI-014|Artifact upload without retention/expiry|Add retention-days to limit artifact storage'
