...
MIGRATESAFE_SQL_PATTERNS+=(
  ...
  'CREATE[[:space:]]+INDEX[[:space:]]+(?!CONCURRENTLY)[A-Za-z_"`\[]+|medium|index_no_concurrent|CREATE INDEX without CONCURRENTLY — Acquires exclusive lock, blocks writes|Use CREATE INDEX CONCURRENTLY to avoid blocking writes during index creation (PostgreSQL). For MySQL, consider pt-online-schema-change.'
