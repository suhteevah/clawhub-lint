...
REGEXGUARD_PE_PATTERNS+=(
  ...
  '\(\?=|medium|PE-004|Lookahead (?=...) used (not supported in POSIX, sed, grep -E)|Replace lookahead with capturing groups and post-match logic if targeting POSIX tools'

  # PE-005: Lookbehind (?<=...) used in a context that may not support it
  '\(\?<=|medium|PE-005|Lookbehind (?<=...) used (not supported in many engines including JS pre-ES2018)|Replace lookbehind with alternative matching strategy for broader engine support'

  # PE-006: Negative lookahead (?!...) used in possibly non-supporting context
  '\(\?!|medium|PE-006|Negative lookahead (?!...) used (not supported in POSIX/BRE tools)|Replace negative lookahead with post-match filtering for POSIX portability'

  # PE-007: Negative lookbehind (?<!...) used -- limited engine support
  '\(\?<!|medium|PE-007|Negative lookbehind (?<!...) used (limited engine support)|Replace negative lookbehind with alternative approach for cross-engine compatibility'

  # PE-008: Named capture group (?P<name>...) -- Python-specific syntax
  '\(\?P<[[:alnum:]]+>|high|PE-008|Python-specific named group (?P<name>...) syntax used|Use (?<name>...) for JS/C# or numbered groups for broader portability'
